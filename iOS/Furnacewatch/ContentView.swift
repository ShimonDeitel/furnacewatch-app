...
struct ContentView: View {
    ...
    @State private var editingEntry: ServiceEntry?
...
struct EntryFormView: View {
    ...
    @State private var stage: String = ServiceEntry.stageOptions.first ?? ""
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Details") {
                    ...
                    Picker("Stage", selection: $stage) {
                        ForEach(ServiceEntry.stageOptions, id: \.self) { Text($0).tag($0) }
                    }
            ...
            .navigationTitle(mode.id == "add" ? "New Service" : "Edit Service")
            .toolbar {
    ...
    private func save() {
        switch mode {
        case .add:
            let entry = ServiceEntry(title: title, stage: stage, date: date, note: note, photoData: photoData)
            store.add(entry)
        case .edit(var entry):
            entry.title = title
            entry.stage = stage
